---
id: "legal.security.pt_BR"
kind: "legal"
locale: "pt_BR"
slug: "seguranca-e-vulnerabilidades"
title: "Segurança e vulnerabilidades"
category_slug: "legal"
category_name: "Legal e privacidade"
position: 9
managed: true
---

Práticas de segurança e canal de divulgação responsável.

Este documento é mantido por `{{LEGAL_ENTITY_NAME}}`{{LEGAL_ENTITY_REGISTRATION_PT}}, com endereço em `{{LEGAL_ENTITY_ADDRESS}}`. O encarregado é `{{LEGAL_DPO_NAME}}`, contatável em `{{PRIVACY_CONTACT_EMAIL}}`.

## Controles e reporte

Usamos segregação de acesso, criptografia quando configurada, trilhas de auditoria, backups e gestão de vulnerabilidades. Relate uma suspeita a `suporte@aifoodmanager.pro`, sem exploração destrutiva ou exposição pública prematura. Inclua impacto e passos mínimos de reprodução, sem dados pessoais reais.

## Referências oficiais

- [Lei Geral de Proteção de Dados Pessoais](https://www.planalto.gov.br/ccivil_03/_ato2015-2018/2018/lei/l13709.htm)
- [Direitos dos titulares — ANPD](https://www.gov.br/anpd/pt-br/assuntos/titular-de-dados-1/direito-dos-titulares)
- [Regulamentações da ANPD](https://www.gov.br/anpd/pt-br/acesso-a-informacao/institucional/atos-normativos/regulamentacoes_anpd)
