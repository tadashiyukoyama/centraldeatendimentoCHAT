...
4. escanear o QR e confirmar a criação de uma única caixa com o número real;
5. enviar texto para o número e confirmar contato, conversa e mensagem;
6. responder pelo Chatwoot e confirmar `source_id`, envio e status;
7. testar uma mídia pequena nos dois sentidos;
8. confirmar que evento repetido não duplica a mensagem;
9. desconectar e reconectar sem criar uma segunda caixa;
10. cancelar uma sessão QR não concluída e comprovar a remoção remota;
11. procurar `apikey`, `jwt_key`, QR e base64 nos logs sanitizados;
12. confirmar saúde de Rails, Sidekiq, Chatwoot PostgreSQL/Redis e dos três
